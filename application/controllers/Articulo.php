...
class Articulo extends CI_Controller{
  ...
  $this->load->model('Vales_consumo_model');
         ...
         $this->data['estado_barra'] =  $this->Vales_consumo_model->get_all_vales_count($this->config->item('Aprobado'),$this->config->item('EnProcesoDeArmado'));
    } 
    ...
    function index()
    {
        ...
        $data['sesion'] = $user;
        if($this->ion_auth->loginCheck() && $this->ion_auth->RolCheck($user->id, $this->config->item('VerArticulos'))){
...
//paginacion
        ...
        $data['articulos'] = $this->Articulo_model->get_all_articulos($params);
    ...
    $user = $this->ion_auth->user()->row();
    $data['sesion'] = $user;
    if($this->ion_auth->loginCheck() && $this->ion_auth->RolCheck($user->id, $this->config->item('AltaArticulos'))){
        ...
        else
        {
            ...
            $data['all_fk_un_med'] = $this->Fk_un_med_model->get_all_fk_un_med();
    ...
    function view($id_articulo){
        ...
        if($this->ion_auth->loginCheck() && $this->ion_auth->RolCheck($user->id, $this->config->item('VerArticulos'))){
            $data['articulo'] = $this->Articulo_model->get_articulo($id_articulo);
              if(isset($data['articulo']['id_articulo'])){

                $data['_view'] = 'articulo/view';
                $this->load->view('layouts/main',$data);
    ...
    {   
         $user = $this->ion_auth->user()->row();
        $data['sesion'] = $user;
        if($this->ion_auth->loginCheck() && $this->ion_auth->RolCheck($user->id, $this->config->item('EditarArticulos'))){
    ...
    function remove($id_articulo)
    {
        ...
        {
            $this->Articulo_model->delete_articulo($id_articulo);
            $this->session->set_flashdata('success','Se ha eliminado correctamente el articulo '.$id_articulo);   
            redirect('articulo/index');
        }
        else
